...
async function getGoalsForEmployee(sheets, employee, location) {
  try {
    ...
    // Find the sheet with form responses (usually contains "Form Responses" or "Responses")
    let sheetName = 'Form Responses 1';
    for (const sheet of spreadsheet.data.sheets) {
      const title = sheet.properties.title;
      if (title.includes('Form Responses') || title.includes('Responses')) {
        sheetName = title;
        break;
      }
    }
    ...
    for (let i = 1; i < rows.length; i++) {
      const row = rows[i];
      console.log(`Checking row ${i}: Employee=${row[1]}, Location=${row[2]}`);
      ...
      if (row[1] === employee && row[2] === location) {
        ...
        if (!latestTimestamp || timestamp > latestTimestamp) {
          ...
          console.log(
            `Found goals for ${employee}: Daily=${latestGoal.daily}, Weekly=${latestGoal.weekly}, Monthly=${latestGoal.monthly}`
          );
        }
      }
    }
