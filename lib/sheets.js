...
function getDateTimeComponents(date) {
  ...
  return {
    timestamp: `${year}-${month}-${day} ${String(hour).padStart(2, '0')}:${String(date.getMinutes()).padStart(2, '0')}:${String(date.getSeconds()).padStart(2, '0')}`,
    hour: hour,
    day: `${year}-${month}-${day}`,
    week: week,
    month: `${year}-${month}`,
...
async function ensureSheetsExist(sheets) {
  try {
    ...
    if (sheetsToCreate.some(req => req.addSheet.properties.title === 'Raw Data')) {
      await sheets.spreadsheets.values.update({
        spreadsheetId: TAP_SPREADSHEET_ID,
        range: 'Raw Data!A1:H1',
        valueInputOption: 'RAW',
        resource: {
          values: [['Timestamp', 'Employee', 'Location', 'Restaurant', 'Hour', 'Day', 'Week', 'Month']]
        }
      });
    }
    
    if (sheetsToCreate.some(req => req.addSheet.properties.title === 'Summary')) {
      await sheets.spreadsheets.values.update({
        spreadsheetId: TAP_SPREADSHEET_ID,
        range: 'Summary!A1:J1',
        valueInputOption: 'RAW',
        resource: {
          values: [['Employee', 'Location', 'Taps Today', 'Daily Goal', '% of Goal', 'Status', 'Taps Week', 'Weekly Goal', 'Taps Month', 'Monthly Goal']]
        }
      });
    }
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      if (row[1] === employee && row[2] === location) {
        ...
        if (row[6] == dateComponents.week) weekCount++;
...
async function updateSummary(sheets, employee, location, tapCounts, goals) {
  try {
    ...
    const dailyPercent = goals.daily > 0 ? Math.round((tapCounts.today / goals.daily) * 100) : 0;
