...
// Spreadsheet IDs - Goals and taps are in the same spreadsheet
const TAP_SPREADSHEET_ID = '1-es6H3GYw8l85hAfSPyKeqmf6Tir6b4kreiovAnqlOc';
const GOALS_SPREADSHEET_ID = '1-es6H3GYw8l85hAfSPyKeqmf6Tir6b4kreiovAnqlOc';
...
async function getGoalsForEmployee(sheets, employee, location) {
  try {
    ...
    // Get all sheet names and use the first one that contains data
    const sheets_list = spreadsheet.data.sheets;
    let sheetName = sheets_list[0].properties.title; // Default to first sheet

    console.log(
      'Available sheets:',
      sheets_list.map((s) => s.properties.title)
    );

    console.log(`Reading goals from sheet: ${sheetName}`);
    ...
    console.log(`Found ${rows.length} rows in goals sheet`);
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      console.log(
        `Comparing: "${row[1]}" === "${employee}" && "${row[2]}" === "${location}"`
      );

      // Match employee and location (case-sensitive exact match)
      if (
        row[1] &&
        row[2] &&
        row[1].trim() === employee &&
        ...
        if (!latestTimestamp || timestamp > latestTimestamp) {
          ...
          console.log(
            `✓ Found matching goals for ${employee}: Daily=${latestGoal.daily}, Weekly=${latestGoal.weekly}, Monthly=${latestGoal.monthly}`
          );
        }
      }
    }
    ...
    if (!latestGoal) {
      console.log(`No goals found for ${employee} at ${location}`);
    }
...
async function updateSummary(sheets, employee, location, tapCounts, goals) {
  try {
    ...
    const weeklyPercent =
      goals.weekly > 0 ? Math.round((tapCounts.week / goals.weekly) * 100) : 0;
    const monthlyPercent =
      goals.monthly > 0
        ? Math.round((tapCounts.month / goals.monthly) * 100)
        : 0;

    const dailyStatus =
      goals.daily > 0 && tapCounts.today >= goals.daily ? 'Met' : 'Not Met';
